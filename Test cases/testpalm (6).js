...
const CRITICAL_ID = '607d8a579beaf0001125fdaa';
const CONTOUR_ID = '61b1fedb17c353005ac48d16';
const KADAVR_MARK = '\u24C0';
const CRITICAL_MARK = '※';
...
async function main() {
    ...
    const params = `include=id,status,stats,bugs,tasks,attributes.${SOX_ID},attributes.${CRITICAL_ID},attributes.${CONTOUR_ID}`;
    ...
    const statuses = tests
        ...
        .map(({id, stats, bugs, tasks, attributes}) => ({
            ...
            isCritical: ((attributes || {})[CRITICAL_ID] || [])[0] === 'yes',
            contour: ((attributes || {})[CONTOUR_ID] || [])[0],
        }));
    const report = {
        project,
        ts: Date.now(),
    ...
    function walk(node) {
        ...
        const tp = status && statusMap.get(name);
        ...
        if (tp && (tp.isSOX || tp.isCritical)) {
            criticalUidSet.add(uid);
            node.name += ` ${CRITICAL_MARK}`;
        }
